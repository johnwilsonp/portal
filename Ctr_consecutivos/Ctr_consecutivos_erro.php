...
$old_error_handler = set_error_handler("Ctr_consecutivos_scriptcase_error_handler");

function Ctr_consecutivos_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="")
{
    ...
    if (isset($_SESSION['scriptcase']['Ctr_consecutivos']['contr_erro']) && $_SESSION['scriptcase']['Ctr_consecutivos']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_Ctr_consecutivos->contr_Ctr_consecutivos->Apl_com_erro = true;
            if ($inicial_Ctr_consecutivos->contr_Ctr_consecutivos->NM_ajax_flag)
            {
                $inicial_Ctr_consecutivos->contr_Ctr_consecutivos->NM_ajax_info['errList']['geral_Ctr_consecutivos'][] = $msg;
                return;
            }
            Ctr_consecutivos_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function Ctr_consecutivos_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['Ctr_consecutivos'])  && '' != $_SESSION['scriptcase']['error_icon']['Ctr_consecutivos'])  ? $_SESSION['scriptcase']['error_icon']['Ctr_consecutivos']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['Ctr_consecutivos']) && '' != $_SESSION['scriptcase']['error_close']['Ctr_consecutivos']) ? $_SESSION['scriptcase']['error_close']['Ctr_consecutivos'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['Ctr_consecutivos']) && '' != $_SESSION['scriptcase']['error_span_title']['Ctr_consecutivos']) ? $_SESSION['scriptcase']['error_span_title']['Ctr_consecutivos'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['Ctr_consecutivos']) && '' != $_SESSION['scriptcase']['error_icon_title']['Ctr_consecutivos']) ? $_SESSION['scriptcase']['error_icon_title']['Ctr_consecutivos'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['Ctr_consecutivos']['error_buffer'] .= ob_get_flush();
}
